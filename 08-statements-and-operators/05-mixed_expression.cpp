...
int main() {
    ...
    double average{0.0};
    // average = static_cast<double>(total / count); // loss of precision
    average = static_cast<double>(total) / count; // recommended casting method.
    // average = (double)total/count;
    // Old-C-Style casting it just casts while static_cast<dataType>
    // double-checks if it can be casted and then casts.
